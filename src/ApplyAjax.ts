...
namespace avtomon {
     ...
     */
    type OkCallback = (response: Response) => Response;
    ...
    /**
     * Тип данных для вставки
     */
    type DataType = Object | Array<Object> | string;
    ...
    export class ApplyAjax {
         ...
         * Конструктор
         *
         * @param {string} _HIDE_CLASS
         * @param {string} _HOST
         * @param {string[]} _ALLOWED_ATTRS
         * @param {ErrorCallback} _DEFAULT_ERROR_CALLBACK
         * @param {Headers} _DEFAULT_HEADERS
         * @param {Params} _DEFAULT_PARAMS
         */
        constructor(
            {
                _HIDE_CLASS = 'clone',
            ...
            this._HIDE_CLASS = _HIDE_CLASS;
            this._HOST = _HOST;
            this._ALLOWED_ATTRS = _ALLOWED_ATTRS;
            this._DEFAULT_ERROR_CALLBACK = _DEFAULT_ERROR_CALLBACK;
            this._DEFAULT_HEADERS = _DEFAULT_HEADERS;
            this._DEFAULT_PARAMS = _DEFAULT_PARAMS;
        }
         ...
         * @param {object | FormData} params - параметры запроса к серверу
         * @param {"GET" | "POST"} type - тип запроса (обычно GET или POST)
         * @param {(response: Response) => Response} callback - функция, отрабатывающая при успешном запросе
         * @param {(e: Error) => Error} callbackError - функция, отрабатывающая при ошибочном результате запроса
         ...
         * @param {(formData: FormData) => Promise<FormData | Error>} before - функция, выполняемая перед отправкой
         * @param {avtomon.OkCallback} callback - коллбэк успешной отправки формы
         * @param {avtomon.ErrorCallback} callbackError - коллбэк неудачной отправки формы
         * @param {(response: Response) => Response} after - эта функция выполняется после успешной отправки формы
         *
         * @returns {Promise<Response | Error>}
         */
        async ajaxSubmit(
            form: HTMLFormElement,
            before: (formData: FormData) => Promise<FormData | Error>,
            callback: OkCallback,
            callbackError?: ErrorCallback,
            ...
            return new Promise<FormData>(async function (resolve, reject): Promise<FormData | Error> {
         ...
         * @param {dataType} data - данные для вставки
         *
         * @returns {HTMLElement | NodeList}
         */
        setMultiData(object: HTMLElement | NodeList, data: DataType = this.data): HTMLElement | NodeList {
         ...
         * @returns {HTMLElement}
         */
        setData(object: HTMLElement, data: DataType = this.data): HTMLElement {
